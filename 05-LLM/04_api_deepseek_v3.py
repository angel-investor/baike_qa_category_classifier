...
# 提前定义提示词
SYSTEM_PROMPT = """
你是一个专业的百科文本数据分类器，你的任务是将输入的百科文本数据分类到以下 233个类别之一：
...
请根据百科文本数据和以下关键词与示例，匹配最相关的类别。如果内容涉及你不知道的数据，优先归为other。
返回 JSON 格式：{"category": "类别", "reason": "分类原因"}

分类规则：
1. 仔细分析给你的百科文本数据
2. 只从上述 233个类别中选择最匹配的一个
3. 返回格式只需返回对应的类别名称
...
请注意，只从上述提供的233个类别中选择一个最合适的进行返回。如果标题与多个类别相关，请选择最相关的一个。请直接返回类别名称，无需额外解释!!!
"""
...
def get_deepseek_res(title: str) -> dict:
    """将百科文本数据分类到指定类别，返回分类结果"""
    # 定义提示词，包含类别说明和示例
    prompt = [
        {"role": "system", "content": SYSTEM_PROMPT},
        {"role": "user", "content": f"百科文本数据：'{title}'，请分类并说明原因。"}
    ]
    ...
    # 返回分类结果，包含类别和原因
    return {
        "category": result.get("category", "other"),  # 默认 社会society
        "reason": result.get("reason", "未明确分类，归为其他类别")
    }
